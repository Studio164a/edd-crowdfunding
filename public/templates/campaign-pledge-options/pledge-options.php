...
if ( isset( $prices[0]['is_donation'] ) && $prices[0]['is_donation'] ) {
	array_shift( $prices );
}
	...
	<ul class="pledge-options">			
		<?php foreach ( $prices as $i => $price ) : 	
			...
			<li data-price="<?php echo edd_sanitize_amount( $price['amount'] )?>" class="pledge-option <?php echo $class ?>">			
				<?php if ( ! $has_limit ) : ?>
					<input type="<?php echo $type ?>" name="edd_options[price_id][]" id="edd_price_option_<?php echo $campaign->ID ?>_<?php echo $price_id ?>" class="pledge-option-input edd_price_option_<?php echo $campaign->ID ?> edd_price_options_input" value="<?php echo $price_id ?>" />
